...
if(isset($_GET['del']) && is_numeric($_GET['del']) && $_GET['del'] > 0)
{
 ...
 if(isset($_POST['subAdd']) || isset($_POST['subEdit']))
 {
  if(isset($_POST['pid']))
  {
    $ar['fullname'] = $_POST['nameEdit'];
    $ar['username'] = $_POST['userEdit'];
    $ar['rank'] = $_POST['rankEdit'];
    $ar['level'] = $_POST['levelEdit'];
    if(strlen($_POST['passEdit']) > 5 )
    {
      $ar['password'] = md5($_POST['passEdit']);
    }
    ...
    if(strlen($_POST['nameEdit']) > 3 )
    {
      ...
      $op->update($db, $ar, array('id'=>$_POST['pid']));
    }

  }
  else
  {
    $ar['fullname'] = $_POST['nameAdd'];
    $ar['username'] = $_POST['userAdd'];
    $ar['rank'] = $_POST['rankAdd'];
    $ar['level'] = $_POST['levelAdd'];
    if(strlen($_POST['passAdd']) > 5 )
    {
      $ar['password'] = md5($_POST['passAdd']);
    }

    if(strlen($_POST['nameAdd']) > 3  && strlen($_POST['passAdd']) > 5)
    {
      $op->insert($db, $ar);
    }
  ...
  <body>
    ...
    <div class="page home-page">
     ...
     <section class="charts">
        ...
        <a  class='btn btn-info' href='mandatesAdd.php'> Add new</a>
      </div>
      </span>
      </div> 
                 ...
                 <table id='mytables' width='100%' style='cellpadding:0px !important' class="" data-pagination="true" data-search="true" data-toggle="tablex">
                  <input type='hidden' name='center' value='<?php echo $id;?>'>
                    ...
                    foreach($data as $d)
                    {
                      ...
                      ?>
                      <?php
                      if(isset($_GET['edit']) && ($d->id == $_GET['edit']))
                      {
                      ?>
                        <tr>
                          <td colspan="5">
                            <table width="100%">
                              <tr>
                                <td width='10px' class='text-center'>Edit</td>
                                <td>
                                  <input name ='pid' type='hidden' value ='<?php echo $_GET['edit'];?>'>
                                  <input type='text' class="form-control" placeholder="Fullname" name='nameEdit' value='<?php echo $d->fullname;?>'>
                                </td>
                                 <td>
                                  <input type='text' class="form-control " placeholder="Username" name='userEdit' value='<?php echo $d->username;?>'>
                                </td>
                                <td>
                                  <input type='text' class="form-control " placeholder="Rank" name='rankEdit' value='<?php echo $d->rank;?>'>
                                </td>
                                <td>
                                  <input type='password' class="form-control" placeholder="password" name='passEdit'>
                                </td>
                                <td>
                                  <select name='levelEdit' class="form-control">
                                  <?php
                                    echo '<option value="'.$d->level.'">'.$d->level.'</option>';
                                ...
                                <td>
                                  <button type="submit" name='subEdit' value='subAdd' class='btn btn-sm'>Change</button>
                                  <a href="users.php"  class='btn btn-sm btn-info'>Cancel</a>
                                </td>
                              </tr>
                            </table>
                           </td>
                      ...
                      <tr id='roz<?php echo $d->id;?>'>          
                        ...
                        <td class='a_row' data-id='<?php echo $d->id;?>'>  
                          <b class='nlot' id='namez<?php echo $d->id;?>'>
                            ...
                            <?php
                            echo ucwords(strtolower($d->fullname));
                            ?> 
                          </b>                       
                        </td>
                         <td>  
                         ...
                         <td>  
                            <?php
                            echo ucwords(strtolower($d->rank));
                            ?>                  
                        </td>
                        <td>  
                            <?php
                            echo ucwords(strtolower($d->level));
                            ?>                  
                        </td> 
                        <td>
                    <div class="btn-group btn-xs col-md-12 float-left" style='padding:0px !important;margin:0px !important; font-size:13px; ' >
                          ...
                          <div class="dropdown-menu">
                            <a class="dropdown-item" href="users.php?edit=<?php echo $d->id;?>">Edit</a>
                            <a class="dropdown-item" href="users.php?act=<?php echo $d->id;?>">Activate</a>
                            <a class="dropdown-item" href="users.php?dct=<?php echo $d->id;?>">Deactivate</a>
                          </div>
                        </div>
                        </td>                  
                      </tr>
                  ...
                  if(!isset($_GET['edit']))
                  {
                    ...
                    <tr>
                      <td colspan="5">
                        <table width="100%">
                          <tr>
                            <td width='10px' class='text-center'>Add</td>
                            <td>
                              <input type='text' class="form-control" placeholder="Fullname" name='nameAdd'>
                            </td>
                            <td>
                              <input type='text' class="form-control" placeholder="Username" name='userAdd'>
                            </td>
                            <td>
                              <input type='text' class="form-control " placeholder="Rank" name='rankAdd'>
                            </td>
                            <td>
                              <input type='password' class="form-control" placeholder="password" name='passAdd'>
                            </td>
                            <td>
                            ...
                            <td>
                              <button type="submit" name='subAdd' value='subAdd' class='btn btn-sm'>Add</button>
