...
{ 
  ...
  $data = $op->selectLogs($center->id, $id, 1, $mid); 
}
        ...
        <div class="container-fluid">
          ...
          <div class="row" style='font-size: 14px'>
            <div class="col-lg-12">
              ...
              <h1>
                 <i class='fa fa-instituition'></i> <?php echo $title_name.'   ('.$id.')';?> Raffle Log
              </h1>
              
              </span>
              <span class='col-md-6 '>
              <div class="float-right btn-group"> 
                  <a  class='btn btn-success' href ="raffleGift.php?id=<?php echo $id;?>"   > Back </a>
                  <button  class='btn btn-success' data-id ="<?php echo $id;?>" data-mid="<?php echo $mid;?>" id='mclear' > Clear All </nutton>
                  <button  class='btn btn-success' data-id ="<?php echo $id;?>" data-mid="<?php echo $mid;?>" id='msms' > Send SMS </button>
              </div>
              </span>
                        ...
                        <td class='a_row text-left' data-id='<?php echo $p->id;?>'>  
                          <b class='nlot' id='namez<?php echo $p->id;?>'>
                            <?php
                              ...
                              echo ($d->logSms == 1)? '<i class="fa fa-envelope text-success"></i>': '<i class="fa fa-envelope text-danger"></i>';
...
$(document).on('click', '#msms', function(){ 
    var r = confirm('Are you sure you want to send SMS to all this individuals . Note that the process is not reversible', 'Yes! Continue', 'No! Cancel');
    if(r == true)
    {
       $('#msms').attr('disabled', true).text('Sending, Please wait');
        var id = $(this).attr('data-id'); 
        var mid = $(this).attr('data-mid');
        var b = {id:id, mid:mid , status:5};
        $.post('utils/loadSmsWin.php', b,  function(dat, status){  
            if(status == 'success')
            {
                    $('#msms').attr('disabled', true).text('SMS Sent!');
                    alert('Please reload to see changes');
            }
        }) ;
     }   
      });
